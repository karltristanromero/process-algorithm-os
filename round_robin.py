...
class RoundRobin(SchedulerBase):
    ...
    def setup_add_process_window(self):
        ...
        # Time Quantum (Always rendered in the Round Robin file—no 'if' wrapper needed!)
        tk.Label(input_frame, text="Time Quantum:", font=('Courier', 12, 'bold'), bg='#2b2b2b', fg='white').grid(row=2, column=0, sticky=tk.W, pady=5)
        self.quantum_entry = tk.Entry(input_frame, font=('Courier', 12, 'bold'), width=20)
    ...
    def add_process(self):
        """Validates entry fields; saves quantum updates independently if process entries are blank."""
        arrival_raw = self.arrival_entry.get().strip()
        burst_raw = self.burst_entry.get().strip()
        quantum_raw = self.quantum_entry.get().strip()

        # Target change context: Quantum is modified but process cells are blank
        if not arrival_raw and not burst_raw:
            try:
                time_quantum = int(quantum_raw)
                if time_quantum <= 0: raise ValueError
                self.time_quantum = time_quantum  # Save quantum update state globally
                self.add_process_window.destroy()  # Exit cleanly
                return
            except ValueError:
                messagebox.showerror("Invalid Input", "Time quantum must be an integer greater than 0.")
                return

        # Regular block execution path when entries are supplied
        try:
    ...
    def edit_process(self):
        ...
        tk.Button(edit_win, text="Save Parameters", bg='#4caf50', fg='white', command=save_changes).grid(row=2, column=0, columnspan=2, pady=10)
    ...
    def open_add_process_window(self):
        """Open popup window and retain previously updated quantum adjustments."""
        ...
        # Forces tracking state synchronization
