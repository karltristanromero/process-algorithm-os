...
class SJFPreemptive(SchedulerBase):
    ...
    def __init__(self, title, width, height):
        """Initialize the Preemptive SJF scheduler."""
        super().__init__(title, width, height)
        self.time_quantum = None  # Not used in SJF but kept for consistency
    ...
    def setup_add_process_window(self):
        """Set up the Add Process popup window for Preemptive SJF."""
    ...
    def add_process(self):
        ...
        try:
            ...
            process = {
                ...
                'remaining_time': burst_time,  # For preemptive SJF
                'completion_time': 0,
                'start_time': -1,
    ...
    def start_simulation(self):
        """Start the Preemptive SJF simulation."""
    ...
    def run_preemptive_sjf(self):
        """Execute the Preemptive SJF algorithm and visualize results."""
        ...
        # Preemptive SJF: at each time unit, pick the process with shortest remaining time
        while completed != n:
            # Find index of process with minimum remaining time among arrived processes
            idx = -1
            ...
            for i in range(n):
                if processes[i]['arrival_time'] <= time and not is_completed[i]:
                    if processes[i]['remaining_time'] < min_remaining:
                        ...
                        idx = i
                    # If remaining time is same, choose the one that arrived earlier (FCFS tie-break)
            ...
            if idx != -1:
                ...
                # Execute for 1 unit time (for visualization granularity)
                ...
                # No process available, increment time (idle)
                time += 1
...
if __name__ == "__main__":
    # For testing
    app = SJFPreemptive("Preemptive SJF Test", 1920, 1080)
    app.setup_main_window()
    # Add some test processes
    app.processes = [
        {'pid': 'P1', 'arrival_time': 0, 'burst_time': 6, 'remaining_time': 6, 'color': '#ff6b6b'},
        {'pid': 'P2', 'arrival_time': 2, 'burst_time': 2, 'remaining_time': 2, 'color': '#4ecdc4'},
        {'pid': 'P3', 'arrival_time': 4, 'burst_time': 8, 'remaining_time': 8, 'color': '#45b7d1'}
    ]
